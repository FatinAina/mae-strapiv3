...
const CurrentEPFMonthlyContribute = ({ navigation, route }) => {
    const [keypadAmount, setKeypadAmount] = useState(
        route?.params?.epfMonthlyContribution
            ? Number(route?.params?.epfMonthlyContribution * 100)
            : ""
    );
    const [rawAmount, setRawAmount] = useState(route?.params?.epfMonthlyContribution ?? 0);
    const [formattedAmount, setFormattedAmount] = useState(
        numberWithCommas(route?.params?.epfMonthlyContribution?.toFixed(2) ?? "")
    );
    ...
    useEffect(() => {
        logEvent(FA_VIEW_SCREEN, {
            [FA_SCREEN_NAME]: "FinancialGoals_Retirement_EPF_MonthlyContribution",
        });
    }, []);
    ...
    function onDoneButtonPress() {
        navigation.navigate(BANKINGV2_MODULE, {
            ...
            params: {
                ...route?.params,
                epfMonthlyContribution: rawAmount,
            },
        });
    }
    ...
    function onClickXButton() {
        navigation.pop(route?.params?.from === GOAL_SIMULATION ? 4 : 3);
                ...
                <React.Fragment>
                    <View style={styles.container}>
                        ...
                        <Typo
                            ...
                            lineHeight={18}
                            style={styles.desc}
                            text="How much is your current monthly contribution (employee &amp; employer)?"
                            textAlign="left"
                        />
...
CurrentEPFMonthlyContribute.propTypes = {
    route: PropTypes.object,
    navigation: PropTypes.object,
};

export default CurrentEPFMonthlyContribute;

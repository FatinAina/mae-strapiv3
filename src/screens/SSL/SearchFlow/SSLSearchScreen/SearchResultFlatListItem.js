...
import { LIGHT_GREY, BLUE, WHITE } from "@constants/colors";

import { commaAndDecimalAdder } from "@utils/dataModel/utilityPartial.1";

import assets from "@assets";

function SearchResultFlatListItem({ item, onPressMerchant, onPressProduct, onPressShowAllStores }) {
    // console.log("SearchResultFlatListItem0", item);
    function onMerchantPress() {
        const { merchantId, shopName } = item;
        onPressMerchant({ merchantId, shopName });
    }
    function onProductPress(id) {
        const { merchantId } = item;
        onPressProduct({ merchantId, id });
    }
    function onShowAllStoresPress() {
        onPressShowAllStores(item?.chainId);
    }
    ...
    const hasChainIdLinkedOutlets = item?.chainId && item?.totalOutlet && item?.totalOutlet > 0;

    let products = [];
    if (isShowAllProduct || hasChainIdLinkedOutlets) {
    ...
    return (
        <View style={styles.searchResultContainer}>
            ...
            {totalProductCount > 2 && !(isShowAllProduct || hasChainIdLinkedOutlets) && (
            ...
            {!!hasChainIdLinkedOutlets && (
                <TouchableOpacity style={styles.viewMoreContainer} onPress={onShowAllStoresPress}>
                    <Typo
                        fontSize={14}
                        lineHeight={24}
                        fontWeight="semi-bold"
                        text={`See All Outlets (${item?.totalOutlet})`}
                        textAlign="right"
                        color={BLUE}
                    />
                </TouchableOpacity>

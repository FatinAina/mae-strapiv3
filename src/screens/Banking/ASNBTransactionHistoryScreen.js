...
import {
    ASNB_TRANSACTION_HISTORY_SCREEN,
    FUNDTRANSFER_MODULE,
    TRANSFER_TAB_SCREEN,
...
export default class ASNBTransactionHistoryScreen extends Component {
    ...
    componentDidMount() {
        GABankingASNB.viewScreenASNBTransactionHistory();
    ...
    _getTransactionHistoryData = async () => {
        try {
            ...
            if (response?.maintenance) {
                ...
                return;
            } else if (response && response.status === 200) return response;
            else return null;
        } catch (error) {
            if (error?.status === "nonetwork") {
                this.props.navigation.replace(WEALTH_ERROR_HANDLING_SCREEN, {
                    error: "NoConnection",
                    fromPage: ASNB_TRANSACTION_HISTORY_SCREEN,
                });
            } else {
                this.props.navigation.replace(WEALTH_ERROR_HANDLING_SCREEN, {

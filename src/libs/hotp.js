...
export default function hotp(key, counter, format) {
    ...
    function hotp_movingfactortohex(count) {
        ...
        let v = decimaltohex(count, 16);
        let decb = new Array();
        lhex = Crypto.util.hexToBytes(v);
        for (let i = 0; i < lhex.length; i++) {
            decb[i] = String.fromCharCode(lhex[i]);
        }
        let retval = new String();
        retval = decb.join('');
        return retval;
    }
    ...
    let hmacBytes = Crypto.HMAC(Crypto.SHA1, Crypto.charenc.Binary.stringToBytes((hotp_movingfactortohex(counter))), Crypto.charenc.Binary.stringToBytes(hotp_hexkeytobytestream(key)));
